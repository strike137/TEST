...
def read_file_from_safe_directory(file_path: str, safe_directory: str) -> str:
    ...
    absolute_file_path = os.path.abspath(os.path.join(safe_directory, file_path))
    absolute_safe_directory = os.path.abspath(safe_directory)

    # Ensure absolute_file_path is under absolute_safe_directory
    if not absolute_file_path.startswith(absolute_safe_directory + os.sep):
