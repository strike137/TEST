...
def run_web_app(requested_url):
    ...
    """
    if requested_url.startswith('/hello/'):
        parts = requested_url.split('/', 2)
        if len(parts) == 3 and parts[1] == 'hello':
            username = parts[2]
            # Directly using the route pattern, mapping may not directly match due to '<username>' placeholder
