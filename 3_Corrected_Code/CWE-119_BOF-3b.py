...
def get_argument_at_index():
    ...
    try:
        ...
        if index < 0 or index >= len(sys.argv) - 1:  # Adjusting the index validity check
            print("Error: Index out of range.")
            return None
        argument = sys.argv[index + 1]  # Adjusting the indexing to sys.argv
        return argument
    except ValueError as e:  # Only catching ValueError for invalid integer conversion
        print(f"Error: {str(e)}")
        return None
