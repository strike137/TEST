...
def read_into_buffer(buffer: bytearray, read_size: int = 10) -> None:
    ...
    """
    input_data = input("Enter at least 10 characters: ")
    if len(input_data) < read_size:
        raise ValueError("Input must contain at least 10 characters.")
    buffer[:read_size] = input_data[:read_size].encode('utf-8')
